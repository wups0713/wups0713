...
int recv_timeo (int sockfd, struct timeval *t_tmp)
{
    // set up
    fd_set rset;
    FD_ZERO (&rset);
    ...
    do
    {
        returnval = select (sockfd + 1, &rset, NULL, NULL, t_tmp);
    } while (returnval < 0 && errno == EINTR);
...
void client (struct sockaddr *servaddr, socklen_t socklen, char *filename)
{
    ...
    // set clock
    struct timeval t_tmp, two_RTT = {0, 500000};
    ...
    while (1)
    {
        ...
        if (recv_timeo (sockfd, &t_tmp) > 0)
        {
            ...
            //two_RTT.tv_sec = 2 * (end.tv_sec - start.tv_sec);
            //two_RTT.tv_usec = 2 * (end.tv_usec - start.tv_usec);
