...
{
	int num = *(int *)arg;
	int a = NEG_INF, b = POS_INF, v = NEG_INF, v_t;

    bot_tmp->seed [num][0] = time (NULL) + num;

	for (int i = num; i < bot_tmp->root->child_count; i = i + THREAD_NUM)
	{
		bot_tmp->root->child[i]->a = a;
		bot_tmp->root->child[i]->b = b;

		v_t = bot_tmp->Mini(bot_tmp->root->child[i], num);
		v = (v > v_t) ? v : v_t;
...
int Bot::Max (Node *node, int num)
{
    ...
    for (int k = 0; k < chess_count; k++)
    {
        ...
        for (int i = 0; i < 4; i++)
            if (can_move (x, y, dir[move_order[i]][0], dir[move_order[i]][1], num))
            {
				...
				{
                    node_count [num][0]++;
					v_t = Mini(do_move_jump(node, NULL, 0, x, y, move_order[i]), num);
					v = (v > v_t) ? v : v_t;
					...
					do_move_jump(node, NULL, 0, x, y, move_order[i]);
        ...
        while (1)
        {
            ...
            for (int i = 0; i < 4; i++)
                if (can_jump (x_t, y_t, dir[jump_order[i]][0], dir[jump_order[i]][1], num))
                {
					...
					{
                        node_count [num][0]++;
						v_t = Mini(do_move_jump(node, node_now, 1, x_t, y_t, jump_order[i]), num);
						v = (v > v_t) ? v : v_t;
						...
						do_move_jump(node, node_now, 1, x_t, y_t, jump_order[i]);
	...
	if (node->depth == MULTITHREAD_ON_OFF)			//on -> 0	off -> depth(never in)
	{
		...
		for (int i = 0; i < THREAD_NUM; i++)
        {
            node_count [i][0] = 0;
            tmp [i] = i;
			pthread_create(&thread[i], NULL, thread_func, (void *)&tmp [i]);
        }
...
int Bot::Mini (Node *node, int num)
{
    ...
    for (int k = 0; k < chess_count; k++)
    {
        ...
        for (int i = 0; i < 4; i++)
            ...
            {
                node_count [num][0]++;
                v_t = Max (do_move_jump(node, NULL, 0, x, y, move_order[i]), num);
                v = (v < v_t)?v:v_t;
        ...
        while (1)
        {
            ...
            for (int i = 0; i < 4; i++)
                ...
                {
                    node_count [num][0]++;
                    v_t = Max (do_move_jump (node, node_now, 1, x_t, y_t, jump_order[i]), num);
                    v = (v < v_t)?v:v_t;
...
Node *Bot::do_move_jump (Node *parent, Node *node, int move_jump, int x, int y, int direct)
{
